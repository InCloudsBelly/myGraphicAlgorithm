...
GLint createVAO4Sphere()
{
	...
	for (unsigned int y = 0; y <= SEGMENTY; ++y)
	{
		for (unsigned int x = 0; x <= SEGMENTX; ++x)
		{
			...
			float PosX = std::cos(XSegment * 2.0f * MyGraphics::PI) * std::sin(YSegment * MyGraphics::PI);
			float PosY = std::cos(YSegment * MyGraphics::PI);
			float PosZ = std::sin(XSegment * 2.0f * MyGraphics::PI) * std::sin(YSegment * MyGraphics::PI);
...
void drawQuad()
{
	glBindVertexArray(_ResourceManager::getOrCreateInstance()->getOrCreateScreenQuadVAO());
...
void drawCube()
{
	glBindVertexArray(_ResourceManager::getOrCreateInstance()->getOrCreateCubeVAO());
...
void drawSphere()
{
	glBindVertexArray(_ResourceManager::getOrCreateInstance()->getOrCretaeSphereVAO());
...
GLvoid ClearTexture(std::shared_ptr<MyGraphics::STexture> vioTexture, GLuint TextureType)
{
...
//************************************************************************************
//Function:
GLvoid genTexture(std::shared_ptr<MyGraphics::STexture> vioTexture)
{
	GLint TextureID;
	...
	{
	case MyGraphics::STexture::ETextureType::Texture2D:
		...
		break;
	case MyGraphics::STexture::ETextureType::Texture2DArray:
		...
		break;
	case MyGraphics::STexture::ETextureType::Texture3D:
		...
		break;
	case MyGraphics::STexture::ETextureType::TextureCubeMap:
		...
		break;
	case MyGraphics::STexture::ETextureType::TextureCubeArray:
		...
		break;
	case MyGraphics::STexture::ETextureType::DepthCubeMap:
		TextureType = GL_TEXTURE_CUBE_MAP;
		glBindTexture(TextureType, TextureID);
		for (int i = 0; i < 6; ++i)
...
//************************************************************************************
//Function:
GLvoid genTextureByBuffer(std::shared_ptr<MyGraphics::STexture> vioTexture, GLint vBuffer)
{
	GLint TextureID;
	glGenTextures(1, &(GLuint&)TextureID);
...
//************************************************************************************
//Function:
GLvoid genGenerateMipmap(std::shared_ptr<MyGraphics::STexture> vioTexture)
{
...
//************************************************************************************
//Function:
void configureDDSTexture(const std::string& vFilePath, gli::texture &voGLITexture, std::shared_ptr<MyGraphics::STexture>& voTexture2D)
{
...
//************************************************************************************
//Function:
void configureCommonTexture(const std::string& vFilePath, std::shared_ptr<MyGraphics::STexture>& voTexture2D)
{
...
//************************************************************************************
//Function:
void configureHDRTexture(const std::string& vFilePath, std::shared_ptr<MyGraphics::STexture>& voTexture2D)
{
	stbi_set_flip_vertically_on_load(true);
	GLint NumChannels = 0, ImageWidth = 0, ImageHeight = 0;
...
GLvoid loadCubeTextureFromFile(const std::vector<std::string>& vFilePath, std::shared_ptr<MyGraphics::STexture> voTexture2D)
{
	if(vFilePath.size()  != 6)
...
//************************************************************************************
//Function:
GLvoid loadTextureFromFile(const std::string& vFilePath, std::shared_ptr<MyGraphics::STexture> voTexture2D)
{
...
//{
//	auto Texture2D = std::make_shared<MyGraphics::STexture>();
//	loadTextureFromFile(vFilePath, Texture2D);
//}

//************************************************************************************
//Function:
GLint genFBO(const std::initializer_list< std::shared_ptr<MyGraphics::STexture>>& vioTextureAttachments)
{
	GLint FBO;
	...
	for (auto& vioTexture : vioTextureAttachments)
	{
		...
		{
		case MyGraphics::STexture::_TextureAttachmentType::DepthArrayTexture:
			...
			break;
		case MyGraphics::STexture::_TextureAttachmentType::DepthTexture:
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, vioTexture->TextureID, 0);
			HasDepthTextureAttachment = GL_TRUE;
			HasStencilTextureAttachment = GL_TRUE;	//FIXME£ºthere is a problem: if set it as GL_FALSE, the stencil render buffer will be added, then result in incomplete fbo 
			break;
		case MyGraphics::STexture::_TextureAttachmentType::CubeDepthTexture:
			glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, vioTexture->TextureID, 0);
			HasDepthTextureAttachment = GL_TRUE;
			HasStencilTextureAttachment = GL_TRUE;	//FIXME£ºthere is a problem: if set it as GL_FALSE, the stencil render buffer will be added, then result in incomplete fbo 
			break;
		case MyGraphics::STexture::_TextureAttachmentType::StencilTexture:
			...
			break;
		case MyGraphics::STexture::_TextureAttachmentType::DepthAndStencilTexture:
	...
	int RenderBufferWidth = vioTextureAttachments.size() > 0 ? (*vioTextureAttachments.begin())->Width : MyGraphics::WINDOW_KEYWORD::WINDOW_WIDTH;
	int RenderBufferHeight = vioTextureAttachments.size() > 0 ? (*vioTextureAttachments.begin())->Height : MyGraphics::WINDOW_KEYWORD::WINDOW_HEIGHT;
...
int captureScreen2Img(const std::string& vFileName, int vQuality)
{
	...
	WindowWidth = MyGraphics::WINDOW_KEYWORD::WINDOW_WIDTH;
	WindowHeight = MyGraphics::WINDOW_KEYWORD::WINDOW_HEIGHT;
...
//************************************************************************************
//Function:
void blitFrameBufferTexture(std::shared_ptr<MyGraphics::STexture> vioSrcTexture, std::shared_ptr<MyGraphics::STexture> vioDestTexture, GLenum vFilter/* = GL_NEAREST*/, std::array<bool, 4> vChannels/* = { true, true, true, true }*/)
{
