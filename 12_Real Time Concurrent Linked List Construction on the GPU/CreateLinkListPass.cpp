...
CreateLinkListPass::CreateLinkListPass(const std::string& vPassName, int vExcutionOrder) : _RenderPass(vPassName, vExcutionOrder)
{
}
...
{
	const int MAX_LIST_NODE = MyGraphics::WINDOW_KEYWORD::getWindowWidth() * MyGraphics::WINDOW_KEYWORD::getWindowHeight();
	...
	m_NodeSSBO = genBuffer(GL_SHADER_STORAGE_BUFFER, MAX_LIST_NODE * sizeof(ListNode) * 32, nullptr, GL_STATIC_DRAW, 0);

	m_HeadTexture = std::make_shared<MyGraphics::STexture>();
	m_HeadTexture->Width = MyGraphics::WINDOW_KEYWORD::getWindowWidth();
	m_HeadTexture->Height = MyGraphics::WINDOW_KEYWORD::getWindowHeight();
	...
	MyGraphics::Camera::setMainCameraPos({ 0.0484748, -0.932904, -2.94066 });
	MyGraphics::Camera::setMainCameraFront({ 0.0811415, -0.153331, 0.984838 });
	m_pShader = std::make_shared<_Shader>("CreateLinkList_VS.glsl", "CreateLinkList_FS.glsl");
	...
	MyGraphics::ResourceManager::registerSharedData("HeadTexture", m_HeadTexture);
	MyGraphics::ResourceManager::registerSharedData("HeadInitSSBO", m_HeadInitSSBO);
	MyGraphics::ResourceManager::registerSharedData("AtomicSSBO", m_AtomicSSBO);
	MyGraphics::ResourceManager::registerSharedData("NodeSSBO", m_NodeSSBO);
...
void CreateLinkListPass::clear()
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_R32UI, MyGraphics::WINDOW_KEYWORD::getWindowWidth(), MyGraphics::WINDOW_KEYWORD::getWindowHeight(), 0, GL_RED_INTEGER, GL_UNSIGNED_INT, nullptr);
