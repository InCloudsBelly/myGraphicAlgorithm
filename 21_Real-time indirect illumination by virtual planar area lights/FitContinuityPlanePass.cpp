...
using namespace std;
CFitContinuityPlanePass::CFitContinuityPlanePass(const std::string& vPassName, int vExcutionOrder) : _RenderPass(vPassName, vExcutionOrder)
{
}
...
{
	m_Block = MyGraphics::ResourceManager::getSharedDataByName<int>("Block");
	...
	MyGraphics::ResourceManager::updateSharedDataByName("PlaneSSBO", ssbo);
	MyGraphics::ResourceManager::updateSharedDataByName("PlaneCountSSBO", count);
	m_pShader = std::make_shared<_Shader>("FitContinuityPlane_CS.glsl");
	...
	m_pShader->setFloatUniformValue("u_Area", MyGraphics::ResourceManager::getSharedDataByName<float>("LightCameraAreaInWorldSpace") / (m_Block * m_Block));
...
void CFitContinuityPlanePass::updateV()
{
	m_pShader->activeShader();
#ifdef _DEBUG
	if (MyGraphics::InputManager::getKeyStatus(GLFW_KEY_K) == GLFW_PRESS && m_OldKeyKStatus != GLFW_PRESS)
	{
		m_DistThresh += 0.01f;
		m_pShader->setFloatUniformValue("u_DistThresh", m_DistThresh);
	...
	if (MyGraphics::InputManager::getKeyStatus(GLFW_KEY_L) == GLFW_PRESS && m_OldKeyLStatus != GLFW_PRESS)
	{
		m_DistThresh -= 0.01f;
		m_pShader->setFloatUniformValue("u_DistThresh", m_DistThresh);
	...
	//
	if (MyGraphics::InputManager::getKeyStatus(GLFW_KEY_H) == GLFW_PRESS && m_OldKeyHStatus != GLFW_PRESS)
	{
	...
	else if (MyGraphics::InputManager::getKeyStatus(GLFW_KEY_H) == GLFW_RELEASE)
		m_OldKeyHStatus = GLFW_RELEASE;

	if (MyGraphics::InputManager::getKeyStatus(GLFW_KEY_J) == GLFW_PRESS && m_OldKeyJStatus != GLFW_PRESS)
	{
		m_NormalThresh -= 0.01f;
		m_pShader->setFloatUniformValue("u_NormalThresh", m_NormalThresh);
		std::cout << m_NormalThresh << std::endl;
	...
	else if (MyGraphics::InputManager::getKeyStatus(GLFW_KEY_J) == GLFW_RELEASE)
