...
void splitTriangle(vec3 vPositions[3], out vec3 TriangleOne[3],out vec3 TriangleTwo[3])
{
	...
	if(maxEdge == 0)
	{
		...
		TriangleOne[2] = vPositions[2];
		TriangleOne[1] = mid;

		TriangleTwo[1] = vPositions[1];
		TriangleTwo[2] = vPositions[2];
		TriangleTwo[0] = mid;
	}
	else if(maxEdge == 1)
	{
...
void sendTriangle(vec3 vTriangle[3],float vArea,float vLevel,int temp, vec3 normal_center)
{
	...
	TempTriangle.mNormals = normal_center;
...
void main()
{
	...
	vec3 normal_center = (vs_gs_in[0].NormalW.xyz +vs_gs_in[1].NormalW.xyz+vs_gs_in[2].NormalW.xyz)/3;
	...
	while(top > 0 && top < TriangleVerticesNum)
	{
		...
		else
		{
			...
			sendTriangle(Triangle,area,level,temp, normal_center);
			temp++;
		}
	}
