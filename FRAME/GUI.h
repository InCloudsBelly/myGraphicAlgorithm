...
class FRAME_DLLEXPORTS _GUI
{
public:
	_GUI();
	_GUI(const std::string& vName, int vExcutionOrder);
	virtual ~_GUI();

	virtual void initV() {}
	virtual void updateV() {}

	bool operator<(const _GUI& vOtherGUI) const;
	...
	const std::shared_ptr<_mFont>& getFontByName(const std::string& vFontName);
	...
	void pushFont(const std::shared_ptr<_mFont>& vFont);	//need to be paired with popFont
	void popFont();
	...
	bool sliderScalar(const std::string& vLable, MyGraphics::EDataType vDataType, void* vValue, const void* vMin, const void* vMax);			//return true when value changed
	bool sliderScalarN(const std::string& vLable, MyGraphics::EDataType vDataType, int vComponentsNum, void* vValue, const void* vMin, const void* vMax);
	bool sliderFloat4(const std::string& vLable, glm::vec4& vioValue, float vMin, float vMax);
	bool verticalSilderScalar(const std::string& vLable, const glm::vec2& vSliderSize, MyGraphics::EDataType vDataType, void* vValue, const void* vMin, const void* vMax);	//return true when value changed
	void horizontalLine();
	void sameLine();
	...
	void popItemWidth();
	bool dragScalar(const std::string& vLabel, MyGraphics::EDataType vDataType, void* vValue, float vDragSpeed, const void* vMin, const void* vMax);
	bool dragScalarN(const std::string& vLabel, MyGraphics::EDataType vDataType, void* vValue, int vComponentsNum, float vDragSpeed, const void* vMin, const void* vMax);
	...
	void pushStyleColor(MyGraphics::EGUIItemColor vItemColor, const glm::vec4& vColor);
...
private:
	...
	std::map<std::string, std::shared_ptr<_mFont>> m_FontMap;
};