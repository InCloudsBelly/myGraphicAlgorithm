...
CTilePakagePass::CTilePakagePass(const std::string& vPassName, int vExcutionOrder) : _RenderPass(vPassName, vExcutionOrder)
{
}
...
{
	m_InputTextureArray = std::make_shared<MyGraphics::STexture>();
		...
		for (int j = 0; j < m_Horizontal; ++j)//ср
			for (int k = 0; k < m_Vertical; ++k)//об
				for (int l = 0; l < m_Vertical; ++l)//вС
				{
					...
					pData = stbi_load(("Textures/tile_" + t + ".bmp").c_str(), &Width, &Height, &Channels, 0);
					m_InputTextureArray->pDataSet.push_back(pData);
					glm::ivec4 edge = glm::ivec4(i, j, k, l);
					m_Edges.push_back(edge);
				}
	m_InputTextureArray->TextureType = MyGraphics::STexture::ETextureType::Texture2DArray;
	...
	MyGraphics::ResourceManager::registerSharedData("TextureArray", m_InputTextureArray);
	MyGraphics::ResourceManager::registerSharedData("TileWidth", MyGraphics::WINDOW_KEYWORD::getWindowWidth() / m_HorizontalNum);
	MyGraphics::ResourceManager::registerSharedData("TileHeight", MyGraphics::WINDOW_KEYWORD::getWindowHeight() / m_VerticalNum);
