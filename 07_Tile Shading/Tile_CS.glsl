...
void main()
{
	...
	barrier();							//此处的barrier可以保证本工作组中的所有线程都完成到这一步，即计算出了正确的深度最大值和最小值。
	...
	{
		uint k = gl_LocalInvocationIndex + i;				//每条线程分别处理u_TotalLightNum /ThreadsNumInTile（256）个光源，一个WorkGroup中有256条线程，将并行地检测所有光源与WorkGroup代表的Tile进行求交。 
