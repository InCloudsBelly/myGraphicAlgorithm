...
{	
	...
	PolygonalLightVertexPosInTangentSpace[4] = PolygonalLightVertexPosInTangentSpace[0];
...
vec2 LTC_Coords(float vCosTheta, float vRoughness)
{
	...
	vec2 Coords = vec2(vRoughness, sqrt(1 - vCosTheta));
	// scale and bias coordinates, for correct filtered lookup
	Coords = Coords * LUT_SCALE + LUT_BIAS;
	return Coords;
}
...
void main()
{
	...
	vec2 UV = LTC_Coords(max(dot(GroundNormal, ViewDir),0), u_Roughness);
	...
	mat3 LTCMatrix = mat3
	(
		vec3(LTCMatrixComponents.x, 0, LTCMatrixComponents.y),
		vec3(0, 1, 0),
		vec3(LTCMatrixComponents.z, 0, LTCMatrixComponents.w)
	);
