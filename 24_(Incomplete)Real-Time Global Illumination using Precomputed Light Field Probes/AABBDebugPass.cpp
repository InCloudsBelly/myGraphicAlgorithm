...
CAABBDebugPass::CAABBDebugPass(const std::string& vPassName, int vExecutionOrder) : _RenderPass(vPassName, vExecutionOrder)
{
}
...
void CAABBDebugPass::initV()
{
	...
	auto AlbedoTextures = MyGraphics::ResourceManager::getSharedDataByName<std::shared_ptr<MyGraphics::STexture>>("OctRadianceTextures");

	m_pShader = std::make_shared<_Shader>("AABBDebug_VS.glsl", "AABBDebug_FS.glsl");
