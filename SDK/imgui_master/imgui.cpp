...
// [SECTION] MAIN USER FACING STRUCTURES (ImGuiStyle, _mGuiIO)
...
/*
 ...
 THIS HOW A SIMPLE RENDERING FUNCTION MAY LOOK LIKE
    ...
    void void MyImGuiRenderFunction(ImDrawData* draw_data)
    {
       ...
       // TODO: Setup _Shader: vertex { float2 pos, float2 uv, u32 color }, fragment _Shader sample color from 1 texture, multiply by vertex color.
 ...
 - 2018/03/08 (1.60) - changed _mFont::DisplayOffset.y to default to 0 instead of +1. Fixed rounding of Ascent/Descent to match TrueType renderer. If you were adding or subtracting to _mFont::DisplayOffset check if your fonts are correctly aligned vertically.
 ...
 A: You can read the 'io.WantCaptureMouse', 'io.WantCaptureKeyboard' and 'io.WantTextInput' flags from the _mGuiIO structure (e.g. if (ImGui::GetIO().WantCaptureMouse) { ... } )
 ...
 A: Use the font atlas to pack them into a single texture:
    (Read misc/fonts/README.txt and the code in ImFontAtlas for more details.)
      ...
      _mGuiIO& io = ImGui::GetIO();
      _mFont* font0 = io.Fonts->AddFontDefault();
      _mFont* font1 = io.Fonts->AddFontFromFileTTF("myfontfile.ttf", size_in_pixels);
      _mFont* font2 = io.Fonts->AddFontFromFileTTF("myfontfile2.ttf", size_in_pixels);
      io.Fonts->GetTexDataAsRGBA32() or GetTexDataAsAlpha8()
...
void _mGuiIO::AddInputCharacter(ImWchar c)
{
...
void _mGuiIO::AddInputCharactersUTF8(const char* utf8_chars)
{
    // We can't pass more wchars than _mGuiIO::InputCharacters[] can hold so don't convert more
    const int wchars_buf_len = sizeof(_mGuiIO::InputCharacters) / sizeof(ImWchar);
...
bool ImGui::DebugCheckVersionAndDataLayout(const char* version, size_t sz_io, size_t sz_style, size_t sz_vec2, size_t sz_vec4, size_t sz_vert)
{
    ...
    if (sz_io    != sizeof(_mGuiIO))       { error = true; IM_ASSERT(sz_io    == sizeof(_mGuiIO)      && "Mismatched struct layout!"); }
...
void ImGui::SetCurrentFont(_mFont* font)
{
    ImGuiContext& g = *GImGui;
...
void ImGui::PushFont(_mFont* font)
{
    ImGuiContext& g = *GImGui;
    if (!font)
        font = GetDefaultFont();
