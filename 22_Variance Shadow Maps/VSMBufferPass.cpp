...
_VSMBufferPass::_VSMBufferPass(const std::string& vPassName, int vExcutionOrder) : _RenderPass(vPassName, vExcutionOrder)
{
}

_VSMBufferPass::~_VSMBufferPass()
{
}

void _VSMBufferPass::initV()
{
	m_pShader = std::make_shared<_Shader>("VSMBuffer_VS.glsl", "VSMBuffer_FS.glsl");
	m_pSponza = std::dynamic_pointer_cast<_Sponza>(MyGraphics::ResourceManager::getGameObjectByName("Sponza"));
	m_pDynamicObjectShader = std::make_shared<_Shader>("VSMBuffer_VS.glsl", "VSMBuffer_FS.glsl");
	...
	TextureDepth->isMipmap = false;
	TextureDepth->InternalFormat = GL_RG32F;
	TextureDepth->ExternalFormat = GL_RG;
	TextureDepth->DataType = GL_FLOAT;
	TextureDepth->Width  = m_RSMResolution;
	TextureDepth->Height = m_RSMResolution;
	TextureDepth->Type4WrapS = GL_CLAMP_TO_EDGE;
	TextureDepth->Type4WrapT = GL_CLAMP_TO_EDGE;
