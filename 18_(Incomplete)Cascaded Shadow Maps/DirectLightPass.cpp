...
void _DirectLightPass::initV()
{
	...
	for (int i = 0; i < 5; ++i) {
		m_pShader->setFloatUniformValue("u_FarPlane[" + std::to_string(i) + "]", MyGraphics::ResourceManager::getSharedDataByName<float>("FarPlane[" + std::to_string(i) + "]"));
	}
...
void _DirectLightPass::updateV()
{
	...
	for (int i = 0; i < Frustum.size(); i++)
	{
		...
		m_pShader->setMat4UniformValue("u_LightVPMatrix[" + std::to_string(i) +"]", glm::value_ptr(m_LightVPMatrix));

	}
	glm::vec3 cameraPos = MyGraphics::Camera::getMainCameraPos();
	m_pShader->setFloatUniformValue("u_CameraPos", cameraPos.x, cameraPos.y, cameraPos.z);
