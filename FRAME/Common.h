...
#define MIN_VALUE 1.175494351e-38F
	...
	struct FRAME_DLLEXPORTS STexture
	{
		...
		enum class _TextureAttachmentType
		{
			ColorTexture,
			DepthTexture,
			CubeDepthTexture,
		...
		_TextureAttachmentType	TextureAttachmentType;
		...
		STexture() : TextureID(-1), InternalFormat(GL_RGBA), ExternalFormat(GL_RGBA), DataType(GL_UNSIGNED_BYTE), Width(WINDOW_KEYWORD::WINDOW_WIDTH),
			...
			Type4MagFilter(GL_LINEAR), isMipmap(GL_FALSE), isSRGBSpace(GL_FALSE), TextureType(ETextureType::Texture2D), TextureAttachmentType(_TextureAttachmentType::ColorTexture), 
			BorderColor{ 1,1,1,1 }, ImageBindUnit(-1), FrameBufferID(-1), AttachmentID(-1)
		{
		}

		bool operator<(const MyGraphics::STexture& vAnotherTexture) const
		{
			return TextureID < vAnotherTexture.TextureID;
		}
	};

	struct FRAME_DLLEXPORTS _PointLight
	{
		glm::vec4 Position;
		glm::vec4 Color;
	};
