...
COpacityShadingPass::COpacityShadingPass(const std::string& vPassName, int vExcutionOrder) : _RenderPass(vPassName, vExcutionOrder)
{
}
...
void COpacityShadingPass::initV()
{
	...
	m_pShader = std::make_shared<_Shader>("OpacityShading_VS.glsl", "OpacityShading_FS.glsl");
	...
	MyGraphics::ResourceManager::registerSharedData("OpacityAlbedoTexture", TextureAlbedo);
	MyGraphics::ResourceManager::registerSharedData("OpacityDepthTexture", TextureDepth);
