...
FRAME_DLLEXPORTS GLvoid genTexture(std::shared_ptr<MyGraphics::STexture> vioTexture/* = MyGraphics::STexture()*/);
FRAME_DLLEXPORTS GLvoid genTextureByBuffer(std::shared_ptr<MyGraphics::STexture> vioTexture, GLint vBuffer);
FRAME_DLLEXPORTS GLvoid genGenerateMipmap(std::shared_ptr<MyGraphics::STexture> vioTexture);
FRAME_DLLEXPORTS GLvoid ClearTexture(std::shared_ptr<MyGraphics::STexture> vioTexture, GLuint TextureType);
FRAME_DLLEXPORTS GLvoid loadTextureFromFile(const std::string& vFilePath, std::shared_ptr<MyGraphics::STexture> voTexture2D);
FRAME_DLLEXPORTS GLvoid loadCubeTextureFromFile(const std::vector<std::string>& vFilePath, std::shared_ptr<MyGraphics::STexture> voTexture2D);
...
FRAME_DLLEXPORTS GLint  genFBO(const std::initializer_list<std::shared_ptr<MyGraphics::STexture>>& vioTextureAttachments);
...
FRAME_DLLEXPORTS void   blitFrameBufferTexture(std::shared_ptr<MyGraphics::STexture> vioSrcTexture, std::shared_ptr<MyGraphics::STexture> vioDestTexture, GLenum vFilter = GL_NEAREST, std::array<bool, 4> vChannels = { true, true, true, true});
