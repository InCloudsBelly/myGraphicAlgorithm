...
CGaussianBlurPass::CGaussianBlurPass(const std::string& vPassName, int vExecutionOrder) : _RenderPass(vPassName, vExecutionOrder)
{
}
...
{
	m_pShader = std::make_shared<_Shader>("GaussianBlur_VS.glsl", "GaussianBlur_FS.glsl");
	m_InputTexture = std::make_shared<MyGraphics::STexture>();
	...
	MyGraphics::ResourceManager::registerSharedData("BluredTexture", m_PingPongTexture[0]);
	MyGraphics::ResourceManager::registerSharedData("MaxMipLevel", m_MaxMipLevel);
	m_InputTexture = MyGraphics::ResourceManager::getSharedDataByName<std::shared_ptr<MyGraphics::STexture>>("WeightAlbedoImage");
	m_pShader->activeShader();
	m_pShader->setTextureUniformValue("u_InputImage", MyGraphics::ResourceManager::getSharedDataByName<std::shared_ptr<MyGraphics::STexture>>("WeightAlbedoImage"));
